...
def test_jit_compiles_and_keeps_interval_order():
    ...
    out = arb_fmpz_poly.arb_fmpz_poly_eval_cubic_batch_jit(coeffs, x)
...
def test_grad_path_for_midpoint():
    def loss(t):
        ...
        x = di.interval(tt, tt)
        out = arb_fmpz_poly.arb_fmpz_poly_eval_cubic(coeffs, x)
...
def test_precision_semantics_wider_at_lower_precision():
    ...
    hi = arb_fmpz_poly.arb_fmpz_poly_eval_cubic_prec(coeffs, x, prec_bits=53)
    lo = arb_fmpz_poly.arb_fmpz_poly_eval_cubic_prec(coeffs, x, prec_bits=20)
