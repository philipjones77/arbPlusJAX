...
def _random_boxes(rng: np.random.Generator, n: int) -> np.ndarray:
    re = _random_intervals(rng, n, 0.8, 2.5)
    im = _random_intervals(rng, n, -0.5, 0.5)
    return np.concatenate([re, im], axis=-1)


def _call_unary(lib, fn_name: str, x: np.ndarray, n_terms: int) -> np.ndarray:
    fn = getattr(lib, fn_name)
    out = np.empty_like(x)
...
def test_acb_dirichlet_parity():
    lib = _load_lib()
    rng = np.random.default_rng(2131)
    s = _random_boxes(rng, 3000)
    n_terms = 64

    c_zeta = _call_unary(lib, "acb_dirichlet_zeta_ref", s, n_terms)
    j_zeta = np.asarray(acb_dirichlet.acb_dirichlet_zeta_batch_jit(jnp.asarray(s), n_terms=n_terms))
    np.testing.assert_allclose(c_zeta, j_zeta, rtol=5e-12, atol=2e-12, equal_nan=True)

    c_eta = _call_unary(lib, "acb_dirichlet_eta_ref", s, n_terms)
    j_eta = np.asarray(acb_dirichlet.acb_dirichlet_eta_batch_jit(jnp.asarray(s), n_terms=n_terms))
    np.testing.assert_allclose(c_eta, j_eta, rtol=5e-12, atol=2e-12, equal_nan=True)
