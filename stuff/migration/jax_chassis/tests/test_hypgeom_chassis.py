...
def test_lgamma_jit_real_and_complex():
    ...
    assert yr.shape == (2,)
    assert yc.shape == (4,)
    assert bool(yr[0] <= yr[1])
    assert bool(yc[0] <= yc[1])
    assert bool(yc[2] <= yc[3])
...
def test_gamma_rgamma_jit_shapes():
    ...
    assert gc.shape == (4,)
    assert rc.shape == (4,)
    assert bool(gr[0] <= gr[1] and rr[0] <= rr[1])
    assert bool(gc[0] <= gc[1] and gc[2] <= gc[3])
    assert bool(rc[0] <= rc[1] and rc[2] <= rc[3])
...
def test_gamma_rgamma_grad_paths():
    ...
    assert bool(jnp.all(jnp.isfinite(g3)))
    assert bool(jnp.all(jnp.isfinite(g4)))
...
def test_erf_family_jit_shapes():
    ...
    assert er.shape == (2,) and cr.shape == (2,) and ir.shape == (2,)
    assert ec.shape == (4,) and cc.shape == (4,) and ic.shape == (4,)
    assert bool(er[0] <= er[1] and cr[0] <= cr[1] and ir[0] <= ir[1])
    assert bool(ec[0] <= ec[1] and ec[2] <= ec[3])
    assert bool(cc[0] <= cc[1] and cc[2] <= cc[3])
    assert bool(ic[0] <= ic[1] and ic[2] <= ic[3])
...
def test_erfinv_erfcinv_shapes_and_grad_paths():
    ...
    assert erfi.shape == (2,)
    assert erfc.shape == (2,)
    assert bool(erfi[0] <= erfi[1])
    assert bool(erfc[0] <= erfc[1])
...
def test_1f1_2f1_jit_shapes():
    ...
    assert r0.shape == (2,) and r1.shape == (2,) and rm.shape == (2,) and r2.shape == (2,)
    assert c0.shape == (4,) and c1.shape == (4,) and cm.shape == (4,) and c2.shape == (4,)
    assert bool(r0[0] <= r0[1] and r1[0] <= r1[1] and rm[0] <= rm[1] and r2[0] <= r2[1])
    assert bool(c0[0] <= c0[1] and c0[2] <= c0[3])
    assert bool(c1[0] <= c1[1] and c1[2] <= c1[3])
    assert bool(cm[0] <= cm[1] and cm[2] <= cm[3])
    assert bool(c2[0] <= c2[1] and c2[2] <= c2[3])
...
def test_u_and_integration_paths():
    ...
    assert ur.shape == (2,) and ui.shape == (2,)
    assert uc.shape == (4,) and uci.shape == (4,)
...
def test_bessel_shapes_and_grad_paths():
    ...
    assert j.shape == (2,) and y.shape == (2,) and i.shape == (2,) and k.shape == (2,)
    ...
    assert jc.shape == (4,) and yc.shape == (4,) and ic.shape == (4,) and kc.shape == (4,)
...
def test_precision_semantics_for_hypgeom_kernels():
    ...
    assert bool(z0_lo[0] <= z0_hi[0] and z0_lo[1] >= z0_hi[1])
...
def test_additional_specials_shapes():
    ...
    assert bool(s[0] <= s[1] and c[0] <= c[1])
...
def test_new_arb_hypgeom_stage2_shapes():
    ...
    assert ai.shape == (2,) and aip.shape == (2,) and bi.shape == (2,) and bip.shape == (2,)

    assert hypgeom.arb_hypgeom_expint(s, x).shape == (2,)
    assert hypgeom.arb_hypgeom_gamma_lower(s, x).shape == (2,)
    assert hypgeom.arb_hypgeom_gamma_upper(s, x).shape == (2,)
    assert hypgeom.arb_hypgeom_beta_lower(a, b, x).shape == (2,)
    assert hypgeom.arb_hypgeom_chebyshev_t(3, x).shape == (2,)
    assert hypgeom.arb_hypgeom_chebyshev_u(3, x).shape == (2,)
    assert hypgeom.arb_hypgeom_laguerre_l(3, a, x).shape == (2,)
    assert hypgeom.arb_hypgeom_hermite_h(4, x).shape == (2,)
    assert hypgeom.arb_hypgeom_legendre_p(3, a, x).shape == (2,)
    assert hypgeom.arb_hypgeom_legendre_q(2, a, x).shape == (2,)
    assert hypgeom.arb_hypgeom_jacobi_p(3, a, b, x).shape == (2,)
    assert hypgeom.arb_hypgeom_gegenbauer_c(3, a, x).shape == (2,)
    assert hypgeom.arb_hypgeom_central_bin_ui(5).shape == (2,)
...
def test_hypgeom_series_and_helpers_shapes():
    ...
    assert hypgeom.arb_hypgeom_rising_ui_jet(x, 3, 4).shape == (4, 2)
    assert hypgeom.arb_hypgeom_gamma_lower_series(s, x, 4).shape == (4, 2)
    assert hypgeom.arb_hypgeom_gamma_upper_series(s, x, 4).shape == (4, 2)
    assert hypgeom.arb_hypgeom_beta_lower_series(a, b, x, 4).shape == (4, 2)
    assert hypgeom.arb_hypgeom_erf_series(x, 4).shape == (4, 2)
    assert hypgeom.arb_hypgeom_ei_series(x, 4).shape == (4, 2)
    s_coef, c_coef = hypgeom.arb_hypgeom_fresnel_series(x, 4)
    assert s_coef.shape == (4, 2) and c_coef.shape == (4, 2)

    ai0, aip0, bi0, bip0 = hypgeom.arb_hypgeom_airy_zero(1)
    assert ai0.shape == (2,) and aip0.shape == (2,) and bi0.shape == (2,) and bip0.shape == (2,)

    f, g = hypgeom.arb_hypgeom_coulomb(a, eta, x)
    assert f.shape == (2,) and g.shape == (2,)

    root, weight = hypgeom.arb_hypgeom_legendre_p_ui_root(4, 2)
    assert root.shape == (2,) and weight.shape == (2,)
