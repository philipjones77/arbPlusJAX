...
def test_rising_ui_real_and_complex_parity():
    ...
    for n in (0, 1, 2, 5, 9, 15):
        ...
        assert np.all(j_real[:, 0] <= j_real[:, 1])
        assert np.all(j_cplx[:, 0] <= j_cplx[:, 1])
        assert np.all(j_cplx[:, 2] <= j_cplx[:, 3])
...
def test_gamma_rgamma_real_and_complex_parity():
    ...
    assert np.all(j_rg[:, 0] <= j_rg[:, 1])
    assert np.all(j_cg[:, 0] <= j_cg[:, 1])
    assert np.all(j_cg[:, 2] <= j_cg[:, 3])
    assert np.all(j_rr[:, 0] <= j_rr[:, 1])
    assert np.all(j_cr[:, 0] <= j_cr[:, 1])
    assert np.all(j_cr[:, 2] <= j_cr[:, 3])
