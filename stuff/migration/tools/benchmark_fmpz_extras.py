...
def main() -> int:
    parser = argparse.ArgumentParser(description="Benchmark fmpz_extras JAX kernels.")
    ...
    a = jnp.asarray(rng.integers(-1000, 1000, size=args.samples, dtype=np.int64))
    b = jnp.asarray(rng.integers(-1000, 1000, size=args.samples, dtype=np.int64))

    if args.which == "add":
        fn = jax.jit(fmpz_extras.fmpz_extras_add_batch)
    else:
        fn = jax.jit(fmpz_extras.fmpz_extras_mul_batch)
    ...
    print(f"fmpz_extras ({args.which}) | samples={args.samples} | time_ms={ms:.2f}")
    _log_run(
        "benchmark_fmpz_extras",
        f"benchmark_fmpz_extras.py --samples {args.samples} --which {args.which}",
        f"time_ms={ms:.2f}",
    )
    return 0
