...
def default_paths() -> tuple[Path | None, Path | None]:
    ...
    lib = _find_lib(build_dir, ["arb_poly_ref.dll", "libarb_poly_ref.dll", "libarb_poly_ref.so", "libarb_poly_ref.dylib"])
    return di, lib
...
def load_lib(di_path: Path, lib_path: Path):
    ...
    fn = lib.arb_poly_eval_cubic_ref
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Compare arb_poly C and JAX kernels.")
    parser.add_argument("--samples", type=int, default=6000)
    args = parser.parse_args()

    di_env = os.getenv("DI_REF_LIB", "")
    lib_env = os.getenv("ARB_POLY_REF_LIB", "")
    ...
    out_j = np.asarray(arb_poly.arb_poly_eval_cubic_batch_jit(jnp.asarray(coeffs), jnp.asarray(x)))
    ok = report("arb_poly_cubic", out_c, out_j, rtol=5e-12, atol=2e-12)

    result = "PASS" if ok else "FAIL"
    print(f"\nresult: {result}")
    _log_run("compare_arb_poly", f"compare_arb_poly.py --samples {args.samples}", f"result={result}")
