...
def main() -> int:
    parser = argparse.ArgumentParser(description="Compare acb_poly C and JAX kernels.")
    ...
    coeffs = random_boxes(rng, 4 * args.samples).reshape(args.samples, 4, 4)
    z = random_boxes(rng, args.samples)

    out_c = np.empty((args.samples, 4), dtype=np.float64)
    fn = lib.acb_poly_eval_cubic_ref
    ...
    ok = report("acb_poly_cubic", out_c, out_j, rtol=5e-12, atol=2e-12)

    result = "PASS" if ok else "FAIL"
    print(f"\nresult: {result}")
    _log_run("compare_acb_poly", f"compare_acb_poly.py --samples {args.samples}", f"result={result}")
