...
from arbjax import acb_elliptic
...
def random_boxes(rng: np.random.Generator, n: int) -> np.ndarray:
    re = random_intervals(rng, n, 0.0, 0.9)
    im = random_intervals(rng, n, -0.2, 0.2)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Compare acb_elliptic C and JAX kernels.")
    ...
    m = random_boxes(rng, args.samples)

    ok = True
    ok &= report(
        "acb_elliptic_k",
        call_unary(lib, "acb_elliptic_k_ref", m),
        np.asarray(acb_elliptic.acb_elliptic_k_batch_jit(jnp.asarray(m))),
        rtol=5e-12,
        atol=2e-12,
    )
    ok &= report(
        "acb_elliptic_e",
        call_unary(lib, "acb_elliptic_e_ref", m),
        np.asarray(acb_elliptic.acb_elliptic_e_batch_jit(jnp.asarray(m))),
        rtol=5e-12,
        atol=2e-12,
    )
    ...
    print(f"\nresult: {result}")
    _log_run("compare_acb_elliptic", f"compare_acb_elliptic.py --samples {args.samples}", f"result={result}")
