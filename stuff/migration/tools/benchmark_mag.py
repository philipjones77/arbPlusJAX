...
def main() -> int:
    parser = argparse.ArgumentParser(description="Benchmark mag JAX kernels.")
    ...
    if args.which == "add":
        fn = jax.jit(mag.mag_add_batch)
    else:
        fn = jax.jit(mag.mag_mul_batch)
    ...
    print(f"mag ({args.which}) | samples={args.samples} | time_ms={ms:.2f}")
    _log_run(
        "benchmark_mag",
        f"benchmark_mag.py --samples {args.samples} --which {args.which}",
        f"time_ms={ms:.2f}",
    )
    return 0
