...
def random_boxes(rng: np.random.Generator, n: int) -> np.ndarray:
    re = random_intervals(rng, n, -0.5, 0.5)
    im = random_intervals(rng, n, -0.3, 0.3)
    return np.concatenate([re, im], axis=-1)


def random_mats(rng: np.random.Generator, n: int) -> np.ndarray:
    return random_boxes(rng, n * 4).reshape(n, 2, 2, 4)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Compare acb_mat C and JAX kernels.")
    ...
    ok &= report(
        "acb_mat_det",
        call_unary(lib, "acb_mat_2x2_det_ref", mats),
        np.asarray(acb_mat.acb_mat_2x2_det_batch_jit(jnp.asarray(mats))),
        rtol=5e-12,
        atol=2e-12,
    )
    ok &= report(
        "acb_mat_trace",
        call_unary(lib, "acb_mat_2x2_trace_ref", mats),
        np.asarray(acb_mat.acb_mat_2x2_trace_batch_jit(jnp.asarray(mats))),
        rtol=5e-12,
        atol=2e-12,
    )
    ...
    print(f"\nresult: {result}")
    _log_run("compare_acb_mat", f"compare_acb_mat.py --samples {args.samples}", f"result={result}")
