...
- JAX / JAX SciPy: resolved via installed JAX or `JAX_REPO` (point‑only reference).
...
- Arb / FLINT is the primary interval reference.
- SciPy and JAX SciPy are float64 point baselines.
- mpmath and Mathematica are high-precision point baselines.
- Boost is an optional extra point baseline.
