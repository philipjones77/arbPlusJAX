...
def _write_report(tool: str, command: str, rows: list[tuple[str, bool, float]], elapsed_s: float, ok: bool) -> Path:
    ...
    lines.append(f"commit: {_git_commit(repo_root)}")
    lines.append(f"platform: {platform.platform()}")
    lines.append(f"elapsed_s: {elapsed_s:.2f}")
    lines.append(f"result: {'PASS' if ok else 'FAIL'}")
    lines.append("")
    lines.append("checks:")
    for name, ok_row, max_diff in rows:
        lines.append(f"- {name}: ok={ok_row} max_abs_diff={max_diff:.3e}")
...
def main() -> int:
    ...
    for n in (0, 1, 2, 5, 9, 15):
        ...
        ok_row, max_diff = report(f"arb_rising_ui n={n}", c_real, j_real, rtol=2e-13)
        rows.append((f"arb_rising_ui n={n}", ok_row, max_diff))
        ok &= ok_row
        ok_row, max_diff = report(f"acb_rising_ui n={n}", c_cplx, j_cplx, rtol=3e-13)
        rows.append((f"acb_rising_ui n={n}", ok_row, max_diff))
        ok &= ok_row
    ...
    ok_row, max_diff = report("arb_lgamma", c_real_lg, j_real_lg, rtol=2e-12, atol=2e-14)
    rows.append(("arb_lgamma", ok_row, max_diff))
    ok &= ok_row
    ok_row, max_diff = report("acb_lgamma", c_cplx_lg, j_cplx_lg, rtol=1e-11, atol=2e-14)
    rows.append(("acb_lgamma", ok_row, max_diff))
    ok &= ok_row
    ...
    ok_row, max_diff = report("arb_gamma", c_real_g, j_real_g, rtol=4e-12, atol=2e-14)
    rows.append(("arb_gamma", ok_row, max_diff))
    ok &= ok_row
    ok_row, max_diff = report("acb_gamma", c_cplx_g, j_cplx_g, rtol=2e-11, atol=2e-14)
    rows.append(("acb_gamma", ok_row, max_diff))
    ok &= ok_row
    ...
    ok_row, max_diff = report("arb_rgamma", c_real_rg, j_real_rg, rtol=4e-12, atol=2e-14)
    rows.append(("arb_rgamma", ok_row, max_diff))
    ok &= ok_row
    ok_row, max_diff = report("acb_rgamma", c_cplx_rg, j_cplx_rg, rtol=2e-11, atol=2e-14)
    rows.append(("acb_rgamma", ok_row, max_diff))
    ok &= ok_row
    ...
    ok_row, max_diff = report("arb_erf", c_erf_r, j_erf_r, rtol=2e-11, atol=2e-14)
    rows.append(("arb_erf", ok_row, max_diff))
    ok &= ok_row
    ok_row, max_diff = report("acb_erf", c_erf_c, j_erf_c, rtol=3e-11, atol=2e-14)
    rows.append(("acb_erf", ok_row, max_diff))
    ok &= ok_row
    ...
    ok_row, max_diff = report("arb_erfc", c_erfc_r, j_erfc_r, rtol=2e-11, atol=2e-14)
    rows.append(("arb_erfc", ok_row, max_diff))
    ok &= ok_row
    ok_row, max_diff = report("acb_erfc", c_erfc_c, j_erfc_c, rtol=3e-11, atol=2e-14)
    rows.append(("acb_erfc", ok_row, max_diff))
    ok &= ok_row
    ...
    ok_row, max_diff = report("arb_erfi", c_erfi_r, j_erfi_r, rtol=2e-11, atol=2e-14)
    rows.append(("arb_erfi", ok_row, max_diff))
    ok &= ok_row
    ok_row, max_diff = report("acb_erfi", c_erfi_c, j_erfi_c, rtol=3e-11, atol=2e-14)
    rows.append(("acb_erfi", ok_row, max_diff))
    ok &= ok_row
    ...
    ok_row, max_diff = report("arb_erfinv", c_erfinv_r, j_erfinv_r, rtol=5e-10, atol=2e-14)
    rows.append(("arb_erfinv", ok_row, max_diff))
    ok &= ok_row
    ok_row, max_diff = report("arb_erfcinv", c_erfcinv_r, j_erfcinv_r, rtol=5e-10, atol=2e-14)
    rows.append(("arb_erfcinv", ok_row, max_diff))
    ok &= ok_row
    ...
    ok_row, max_diff = report("arb_0f1_reg", c_0f1_r, j_0f1_r, rtol=2e-11, atol=2e-14)
    rows.append(("arb_0f1_reg", ok_row, max_diff))
    ok &= ok_row
    ok_row, max_diff = report("acb_0f1_reg", c_0f1_c, j_0f1_c, rtol=4e-11, atol=2e-14)
    rows.append(("acb_0f1_reg", ok_row, max_diff))
    ok &= ok_row
    ...
    ok_row, max_diff = report("arb_m_reg", c_m_r, j_m_r, rtol=2e-11, atol=2e-14)
    rows.append(("arb_m_reg", ok_row, max_diff))
    ok &= ok_row
    ok_row, max_diff = report("acb_m_reg", c_m_c, j_m_c, rtol=4e-11, atol=2e-14)
    rows.append(("acb_m_reg", ok_row, max_diff))
    ok &= ok_row
    ...
    ok_row, max_diff = report("arb_1f1", c_1f1_r, j_1f1_r, rtol=2e-11, atol=2e-14)
    rows.append(("arb_1f1", ok_row, max_diff))
    ok &= ok_row
    ok_row, max_diff = report("acb_1f1", c_1f1_c, j_1f1_c, rtol=4e-11, atol=2e-14)
    rows.append(("acb_1f1", ok_row, max_diff))
    ok &= ok_row
    ...
    ok_row, max_diff = report("arb_1f1_reg", c_1f1r_r, j_1f1r_r, rtol=2e-11, atol=2e-14)
    rows.append(("arb_1f1_reg", ok_row, max_diff))
    ok &= ok_row
    ok_row, max_diff = report("acb_1f1_reg", c_1f1r_c, j_1f1r_c, rtol=4e-11, atol=2e-14)
    rows.append(("acb_1f1_reg", ok_row, max_diff))
    ok &= ok_row
    ...
    ok_row, max_diff = report("arb_2f1", c_2f1_r, j_2f1_r, rtol=3e-11, atol=2e-14)
    rows.append(("arb_2f1", ok_row, max_diff))
    ok &= ok_row
    ok_row, max_diff = report("acb_2f1", c_2f1_c, j_2f1_c, rtol=5e-11, atol=2e-14)
    rows.append(("acb_2f1", ok_row, max_diff))
    ok &= ok_row
    ...
    ok_row, max_diff = report("arb_2f1_reg", c_2f1r_r, j_2f1r_r, rtol=3e-11, atol=2e-14)
    rows.append(("arb_2f1_reg", ok_row, max_diff))
    ok &= ok_row
    ok_row, max_diff = report("acb_2f1_reg", c_2f1r_c, j_2f1r_c, rtol=5e-11, atol=2e-14)
    rows.append(("acb_2f1_reg", ok_row, max_diff))
    ok &= ok_row
    ...
    ok_row, max_diff = report("arb_1f1_int", c_1f1i_r, j_1f1i_r, rtol=3e-11, atol=2e-14)
    rows.append(("arb_1f1_int", ok_row, max_diff))
    ok &= ok_row
    ok_row, max_diff = report("acb_1f1_int", c_1f1i_c, j_1f1i_c, rtol=5e-11, atol=2e-14)
    rows.append(("acb_1f1_int", ok_row, max_diff))
    ok &= ok_row
    ...
    ok_row, max_diff = report("arb_2f1_int", c_2f1i_r, j_2f1i_r, rtol=3e-11, atol=2e-14)
    rows.append(("arb_2f1_int", ok_row, max_diff))
    ok &= ok_row
    ok_row, max_diff = report("acb_2f1_int", c_2f1i_c, j_2f1i_c, rtol=5e-11, atol=2e-14)
    rows.append(("acb_2f1_int", ok_row, max_diff))
    ok &= ok_row
    ...
    ok_row, max_diff = report("arb_u", c_u_r, j_u_r, rtol=5e-10, atol=2e-14)
    rows.append(("arb_u", ok_row, max_diff))
    ok &= ok_row
    ok_row, max_diff = report("acb_u", c_u_c, j_u_c, rtol=5e-10, atol=2e-14)
    rows.append(("acb_u", ok_row, max_diff))
    ok &= ok_row
    ...
    ok_row, max_diff = report("arb_u_int", c_ui_r, j_ui_r, rtol=5e-10, atol=2e-14)
    rows.append(("arb_u_int", ok_row, max_diff))
    ok &= ok_row
    ok_row, max_diff = report("acb_u_int", c_ui_c, j_ui_c, rtol=5e-10, atol=2e-14)
    rows.append(("acb_u_int", ok_row, max_diff))
    ok &= ok_row
    ...
    ok_row, max_diff = report("arb_bessel_j", c_bj_r, j_bj_r, rtol=2e-11, atol=2e-14)
    rows.append(("arb_bessel_j", ok_row, max_diff))
    ok &= ok_row
    ok_row, max_diff = report("arb_bessel_y", c_by_r, j_by_r, rtol=2e-11, atol=2e-14)
    rows.append(("arb_bessel_y", ok_row, max_diff))
    ok &= ok_row
    ok_row, max_diff = report("arb_bessel_i", c_bi_r, j_bi_r, rtol=2e-11, atol=2e-14)
    rows.append(("arb_bessel_i", ok_row, max_diff))
    ok &= ok_row
    ok_row, max_diff = report("arb_bessel_k", c_bk_r, j_bk_r, rtol=2e-11, atol=2e-14)
    rows.append(("arb_bessel_k", ok_row, max_diff))
    ok &= ok_row
    ...
    ok_row, max_diff = report("acb_bessel_j", c_bj_c, j_bj_c, rtol=4e-11, atol=2e-14)
    rows.append(("acb_bessel_j", ok_row, max_diff))
    ok &= ok_row
    ok_row, max_diff = report("acb_bessel_y", c_by_c, j_by_c, rtol=4e-11, atol=2e-14)
    rows.append(("acb_bessel_y", ok_row, max_diff))
    ok &= ok_row
    ok_row, max_diff = report("acb_bessel_i", c_bi_c, j_bi_c, rtol=4e-11, atol=2e-14)
    rows.append(("acb_bessel_i", ok_row, max_diff))
    ok &= ok_row
    ok_row, max_diff = report("acb_bessel_k", c_bk_c, j_bk_c, rtol=4e-11, atol=2e-14)
    rows.append(("acb_bessel_k", ok_row, max_diff))
    ok &= ok_row

    print(f"\nresult: {'PASS' if ok else 'FAIL'}")
    elapsed = time.perf_counter() - start_time
    cmd = f"compare_hypgeom.py --samples-real {args.samples_real} --samples-complex {args.samples_complex} --bessel-real-mode {args.bessel_real_mode}"
    _write_report("compare_hypgeom", cmd, rows, elapsed, ok)
    _log_run("compare_hypgeom", cmd, notes=f"result={'PASS' if ok else 'FAIL'}")
