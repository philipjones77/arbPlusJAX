...
def main() -> int:
    parser = argparse.ArgumentParser(description="Compare arb_calc C and JAX kernels.")
    ...
    a = random_intervals(rng, args.samples, -0.5, 0.5)
    b = random_intervals(rng, args.samples, 0.2, 1.0)
    n_steps = args.steps
    ...
    ok &= report(
        ...
        np.asarray(arb_calc.arb_calc_integrate_line_batch_jit(jnp.asarray(a), jnp.asarray(b), integrand="exp", n_steps=n_steps)),
        rtol=5e-12,
        atol=2e-12,
    )
    ok &= report(
        ...
        np.asarray(arb_calc.arb_calc_integrate_line_batch_jit(jnp.asarray(a), jnp.asarray(b), integrand="sin", n_steps=n_steps)),
        rtol=5e-12,
        atol=2e-12,
    )
    ok &= report(
        ...
        np.asarray(arb_calc.arb_calc_integrate_line_batch_jit(jnp.asarray(a), jnp.asarray(b), integrand="cos", n_steps=n_steps)),
        rtol=5e-12,
        atol=2e-12,
    )
    ...
    print(f"\nresult: {result}")
    _log_run("compare_arb_calc", f"compare_arb_calc.py --samples {args.samples} --steps {args.steps}", f"result={result}")
