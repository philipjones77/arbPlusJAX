...
SOURCE_NAME_RE = re.compile(r"^(?P<repo>[A-Za-z0-9._-]+)_source_(?P<date>\d{4}-\d{2}-\d{2})\.zip$")
...
def is_valid_source_zip_name(filename: str, repo_name: str) -> bool:
    m = SOURCE_NAME_RE.fullmatch(filename)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Package the arbPlusJAX repo into a source zip file.")
    parser.add_argument(
        "--output",
        default=None,
