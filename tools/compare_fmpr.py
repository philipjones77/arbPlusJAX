...
def main() -> int:
    parser = argparse.ArgumentParser(description="Compare fmpr C and JAX kernels.")
    ...
    ok_add = np.allclose(out_add, j_add)
    ok_mul = np.allclose(out_mul, j_mul)
    ok = ok_add and ok_mul

    print(f"fmpr_add | ok={ok_add}")
    print(f"fmpr_mul | ok={ok_mul}")
    result = "PASS" if ok else "FAIL"
    print(f"\nresult: {result}")
    _log_run("compare_fmpr", f"compare_fmpr.py --samples {args.samples}", f"result={result}")
