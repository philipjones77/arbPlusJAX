...
def _make_wrapper(name: str, base_fn: Callable[..., jax.Array], kernel_fn: Callable[..., jax.Array]) -> Callable[..., jax.Array]:
    ...
    def rig_fn(*args, prec_bits: int, **kwargs):
        ...
        if name.startswith("dirichlet_zeta"):
            return dirichlet.dirichlet_zeta_rigorous(*args, **kwargs)
        if name.startswith("dirichlet_eta"):
            return dirichlet.dirichlet_eta_rigorous(*args, **kwargs)
...
for _mod in (acb_dirichlet, dirichlet):
