...
def _make_wrapper(name: str, base_fn: Callable[..., jax.Array], kernel_fn: Callable[..., jax.Array]) -> Callable[..., jax.Array]:
    ...
    def rig_fn(*args, prec_bits: int, **kwargs):
        if name.startswith("acb_modular_j"):
            return acb_modular.acb_modular_j_rigorous(*args, **kwargs)
        if name.startswith("acb_elliptic_k"):
            return acb_elliptic.acb_elliptic_k_rigorous(*args, **kwargs)
        if name.startswith("acb_elliptic_e"):
            return acb_elliptic.acb_elliptic_e_rigorous(*args, **kwargs)
...
for _mod in (acb_modular, acb_elliptic):
