...
from . import acb_poly
from . import arb_poly
from . import arb_fmpz_poly
...
def _make_wrapper(name: str, base_fn: Callable[..., jax.Array], kernel_fn: Callable[..., jax.Array]) -> Callable[..., jax.Array]:
    ...
    def rig_fn(*args, prec_bits: int, **kwargs):
        if is_acb:
            if name.startswith("acb_poly_eval_cubic"):
                return acb_poly.acb_poly_eval_cubic_rigorous(*args, **kwargs)
            return wc.rigorous_acb_kernel(kernel_fn, args, prec_bits, **kwargs)
        if name.startswith("arb_poly_eval_cubic"):
            return arb_poly.arb_poly_eval_cubic_rigorous(*args, **kwargs)
        if name.startswith("arb_fmpz_poly_eval_cubic"):
            return arb_fmpz_poly.arb_fmpz_poly_eval_cubic_rigorous(*args, **kwargs)
...
for _mod in (acb_poly, arb_poly, arb_fmpz_poly):
