...
def _as_coeffs(coeffs: jax.Array) -> jax.Array:
    ...
    checks.check_tail_shape(arr, (4, 2), "arb_poly._as_coeffs")
    return arr
...
def arb_poly_eval_cubic_rigorous(coeffs: jax.Array, x: jax.Array) -> jax.Array:
    coeffs = _as_coeffs(coeffs)
    x = di.as_interval(x)
    c0 = coeffs[..., 0, :]
    c1 = coeffs[..., 1, :]
...
def arb_poly_eval_cubic_batch_rigorous(coeffs: jax.Array, x: jax.Array) -> jax.Array:
    coeffs = _as_coeffs(coeffs)
    x = di.as_interval(x)
    return jax.vmap(arb_poly_eval_cubic_rigorous)(coeffs, x)
...
for _name in dir(_smi):
    if _name in globals():
        continue
    if any(_name.startswith(p) for p in ['arb_poly_', '_arb_poly_']):
