...
def test_cubesselk_point_fallback_matches_hypgeom():
    ...
    out = cubesselk.cuda_besselk_point(nu, z)
...
def test_cubesselk_modes_shapes_and_containment():
    ...
    basic = cubesselk.cuda_besselk(nu, z, mode="basic", prec_bits=80)
    rig = cubesselk.cuda_besselk(nu, z, mode="rigorous", prec_bits=80)
    ad = cubesselk.cuda_besselk(nu, z, mode="adaptive", prec_bits=80)
...
def test_api_dispatch_for_cubesselk_point_and_interval():
    ...
    y_pt = api.eval_point("cuda_besselk", nu_pt, z_pt)
    _check(y_pt.shape == (2,))

    nu_iv = jnp.stack([nu_pt - 1e-6, nu_pt + 1e-6], axis=-1)
    z_iv = jnp.stack([z_pt - 1e-6, z_pt + 1e-6], axis=-1)
    y_iv = api.eval_interval("cuda_besselk", nu_iv, z_iv, mode="basic", prec_bits=80)
    _check(y_iv.shape == (2, 2))

    y_b = api.eval_point_batch("cuda_besselk", nu_pt, z_pt)
    _check(y_b.shape == (2,))
