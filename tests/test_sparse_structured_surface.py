...
def test_srb_sparse_structured_factorizations_do_not_dense_fallback(monkeypatch: pytest.MonkeyPatch):
    dense = jnp.array(
        [
            [6.0, 1.0, 0.0],
            [1.0, 5.0, 0.5],
    ...
    def _forbid_dense(*args, **kwargs):
        raise AssertionError("srb_mat_to_dense should not be used by sparse structured SPD paths")
    ...
    chol = srb_mat.srb_mat_cho(sparse)
    ldl_l, ldl_d = srb_mat.srb_mat_ldl(sparse)
    chol_dense = srb_mat.srb_mat_csr_to_dense(chol)
    ldl_l_dense = srb_mat.srb_mat_csr_to_dense(ldl_l)

    _check(bool(jnp.allclose(chol_dense @ chol_dense.T, dense, rtol=1e-8, atol=1e-8)))
    _check(bool(jnp.allclose(ldl_l_dense @ jnp.diag(ldl_d) @ ldl_l_dense.T, dense, rtol=1e-8, atol=1e-8)))
    _check(bool(jnp.all(ldl_d > 0.0)))


def test_scb_sparse_structured_factorizations_do_not_dense_fallback(monkeypatch: pytest.MonkeyPatch):
    base = jnp.array(
        [
            [2.0 + 0.0j, 1.0 - 0.25j, 0.0 + 0.0j],
            [0.0 + 0.0j, 1.5 + 0.0j, 0.4 + 0.1j],
            [0.25 - 0.1j, 0.0 + 0.0j, 2.25 + 0.0j],
        ],
        dtype=jnp.complex128,
    )
    dense = jnp.conj(base.T) @ base + jnp.eye(3, dtype=jnp.complex128)
    sparse = scb_mat.scb_mat_from_dense_csr(dense)

    def _forbid_dense(*args, **kwargs):
        raise AssertionError("scb_mat_to_dense should not be used by sparse structured HPD paths")
    ...
    chol = scb_mat.scb_mat_cho(sparse)
    ldl_l, ldl_d = scb_mat.scb_mat_ldl(sparse)
    chol_dense = scb_mat.scb_mat_csr_to_dense(chol)
    ldl_l_dense = scb_mat.scb_mat_csr_to_dense(ldl_l)

    _check(bool(jnp.allclose(chol_dense @ jnp.conj(chol_dense.T), dense, rtol=1e-8, atol=1e-8)))
    _check(bool(jnp.allclose(ldl_l_dense @ jnp.diag(ldl_d) @ jnp.conj(ldl_l_dense.T), dense, rtol=1e-8, atol=1e-8)))
    _check(bool(jnp.all(jnp.real(ldl_d) > 0.0)))
