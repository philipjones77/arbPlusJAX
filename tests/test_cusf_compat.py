...
def test_cusf_point_bessel_matches_internal_eval():
    ...
    y = cusf_compat.Cusf_jv(v, x, mode="point")
...
def test_cusf_four_modes_kv_and_derivatives():
    ...
    b = cusf_compat.Cusf_kv(v, x, mode="basic", prec_bits=80)
    r = cusf_compat.Cusf_kv(v, x, mode="rigorous", prec_bits=80)
    a = cusf_compat.Cusf_kv(v, x, mode="adaptive", prec_bits=80)
    _check(bool(di.contains(r, b)))
    _check(bool(di.contains(a, b)))

    db = cusf_compat.Cusf_kvp(v, x, mode="basic", prec_bits=80)
    dr = cusf_compat.Cusf_kvp(v, x, mode="rigorous", prec_bits=80)
    da = cusf_compat.Cusf_kvp(v, x, mode="adaptive", prec_bits=80)
