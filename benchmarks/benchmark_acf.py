...
def main() -> int:
    parser = argparse.ArgumentParser(description="Benchmark acf JAX kernels.")
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("experiments/benchmarks/outputs/scalar/benchmark_acf.json"),
    )
    args = parser.parse_args()

    rng = np.random.default_rng(2183)
    a = jnp.asarray(rng.normal(size=args.samples) + 1j * rng.normal(size=args.samples))
    b = jnp.asarray(rng.normal(size=args.samples) + 1j * rng.normal(size=args.samples))

    if args.which == "add":
        fn = jax.jit(acf.acf_add_batch)
    else:
        fn = jax.jit(acf.acf_mul_batch)
    ...
    report = BenchmarkReport(
        benchmark_name="benchmark_acf.py",
    ...
    print(f"acf ({args.which}) | samples={args.samples} | warm_time_ms={ms:.2f}")
    _log_run(
        "benchmark_acf",
        f"benchmark_acf.py --samples {args.samples} --which {args.which} --runs {args.runs}",
