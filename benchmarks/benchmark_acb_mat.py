...
def _random_mats(rng: np.random.Generator, n: int) -> np.ndarray:
    re = rng.uniform(-0.5, 0.5, size=(n, 2, 2, 2))
    im = rng.uniform(-0.3, 0.3, size=(n, 2, 2, 2))
    lo_re = np.minimum(re[..., 0], re[..., 1])
    hi_re = np.maximum(re[..., 0], re[..., 1])
    lo_im = np.minimum(im[..., 0], im[..., 1])
    hi_im = np.maximum(im[..., 0], im[..., 1])
    return np.stack([lo_re, hi_re, lo_im, hi_im], axis=-1).astype(np.float64)


def main() -> int:
    parser = argparse.ArgumentParser(description="Benchmark acb_mat JAX kernels.")
    ...
    if args.which == "det":
        fn = jax.jit(acb_mat.acb_mat_2x2_det_batch)
    else:
        fn = jax.jit(acb_mat.acb_mat_2x2_trace_batch)
    ...
    print(f"acb_mat ({args.which}) | samples={args.samples} | time_ms={ms:.2f}")
    _log_run(
        "benchmark_acb_mat",
        f"benchmark_acb_mat.py --samples {args.samples} --which {args.which}",
        f"time_ms={ms:.2f}",
    )
    return 0
