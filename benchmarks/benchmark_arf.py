...
def main() -> int:
    parser = argparse.ArgumentParser(description="Benchmark arf JAX kernels.")
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("experiments/benchmarks/outputs/scalar/benchmark_arf.json"),
    )
    ...
    if args.which == "add":
        fn = jax.jit(arf.arf_add_batch)
    else:
        fn = jax.jit(arf.arf_mul_batch)
    ...
    report = BenchmarkReport(
        benchmark_name="benchmark_arf.py",
    ...
    print(f"arf ({args.which}) | samples={args.samples} | warm_time_ms={ms:.2f}")
    _log_run(
        "benchmark_arf",
        f"benchmark_arf.py --samples {args.samples} --which {args.which} --runs {args.runs}",
