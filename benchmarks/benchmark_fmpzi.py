...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("experiments/benchmarks/outputs/scalar/benchmark_fmpzi.json"),
    )
    ...
    report = BenchmarkReport(
        benchmark_name="benchmark_fmpzi.py",
    ...
    print(f"fmpzi ({args.which}) | samples={args.samples} | warm_time_ms={ms:.2f}")
    _log_run(
        "benchmark_fmpzi",
        f"benchmark_fmpzi.py --samples {args.samples} --which {args.which} --runs {args.runs}",
