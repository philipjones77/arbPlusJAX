...
def _random_boxes(rng: np.random.Generator, n: int) -> np.ndarray:
    re = rng.uniform(-0.5, 0.5, size=(n, 2))
    im = rng.uniform(-0.3, 0.3, size=(n, 2))
...
def _bench(fn, *args) -> tuple[float, float, float]:
    ...
    alt_z = args[1][: max(1, args[1].shape[0] // 2)]
    start = time.perf_counter()
    recomp = fn(alt_coeffs, alt_z)
    _block(recomp)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Benchmark acb_poly JAX kernels.")
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("experiments/benchmarks/outputs/poly/benchmark_acb_poly.json"),
    )
    ...
    coeffs = jnp.asarray(_random_boxes(rng, 4 * sample_count).reshape(sample_count, 4, 4), dtype=real_dtype)
    z = jnp.asarray(_random_boxes(rng, sample_count), dtype=real_dtype)

    fn = jax.jit(acb_poly.acb_poly_eval_cubic_batch)
    cold_s, warm_s, recompile_s = _bench(fn, coeffs, z)

    print(f"acb_poly (cubic) | samples={sample_count} | warm_time_ms={warm_s * 1000.0:.2f}")

    report = BenchmarkReport(
        benchmark_name="benchmark_acb_poly.py",
        ...
        records=(
            BenchmarkRecord(
                ...
                notes="Legacy complex polynomial benchmark normalized onto the shared benchmark schema; stdout remains summary-style for notebook compatibility.",
            ),
        ),
        ...
        notes="acb_poly cubic benchmark with explicit dtype and requested runtime mode controls.",
    )
